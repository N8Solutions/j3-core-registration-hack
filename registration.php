...
class UsersModelRegistration extends JModelForm
{
	...
	public function register($temp)
	{
		...
		//
		// BEGIN: ADD CUSTOM FIELDS FROM REGISTRATION FORM TO ADMIN EMAIL NOTIFICATION
		//

		// Assuming $data['email'] is the email of the newly registered user and is available
		$userId = JFactory::getUser($data['email'])->id; // Ensure correct retrieval of user ID

		// Code to retrieve custom fields based on known group IDs
		$db = JFactory::getDbo();
		$groupIds = [1, 2]; // Assuming IDs are known and static, adjust as necessary

		$query = $db->getQuery(true);
		$query->select('fv.field_id, fv.value, f.title')
			  ->from($db->quoteName('#__fields_values', 'fv'))
			  ->join('INNER', $db->quoteName('#__fields', 'f') . ' ON (fv.field_id = f.id)')
			  ->where($db->quoteName('fv.item_id') . ' = ' . $db->quote($userId))
			  ->where($db->quoteName('f.group_id') . ' IN (' . implode(',', $db->quote($groupIds)) . ')');
		$db->setQuery($query);
		...
		if (!empty($results)) {
			foreach ($results as $result) {
				$customFieldsStr .= $result->title . ": " . $result->value . "\n";
			}
		}

		// Construct the email body with custom fields data appended to the end
		...
		//
		// END: ADD CUSTOM FIELDS FROM REGISTRATION FORM TO ADMIN EMAIL NOTIFICATION
		//
